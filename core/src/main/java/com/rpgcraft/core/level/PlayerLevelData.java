 ...
 * <p>
 * 通过 {@link #setRegistry(ILevelRegistry)} 注入经验表注册中心，
 * 由 leveling 模块在初始化时调用。
 */
public class PlayerLevelData {

    /** 经验表注册中心（由 leveling 模块注入） */
    ...
    /**
     * 注入经验表注册中心（由 leveling 模块的 LevelManager.init() 调用）
     *
     * @param reg 经验表注册中心实例
     */
    public static void setRegistry(ILevelRegistry reg) {
        registry = reg;
    }
     ...
     * 获取经验表注册中心
     */
    private static ILevelRegistry getRegistry() {
        if (registry == null) {
            throw new IllegalStateException("PlayerLevelData 注册中心未设置，请先由 leveling 模块初始化");
        }
        return registry;
    }
    ...
    public void setLevel(int level) {
        this.level = Math.clamp(level, 1, getRegistry().getMaxLevel());
    }
    ...
    public int getExpForNextLevel() {
        ...
        return getRegistry().getExpForLevel(level);
    }
    ...
    public boolean addExperience(int amount) {
        ...
        int maxLevel = getRegistry().getMaxLevel();
        int oldLevel = level;
        ...
        while (level < maxLevel) {
            int required = getRegistry().getExpForLevel(level);
