...
public class InventoryAttributePanel {
    ...
    // 暴击率等级颜色
    private static final int COLOR_CRIT_TIER1  = 0xFFFFA500;    // 橙色 (101-200%)
    private static final int COLOR_CRIT_TIER2  = 0xFFFF4444;    // 红色 (201-300%)
     ...
     * 判断屏幕是否为受支持的背包界面
     * <p>
     * 同时支持生存模式 {@link InventoryScreen} 和创造模式 {@link CreativeModeInventoryScreen}。
     * 两者在 Minecraft 26.1.2 中是独立的类，无继承关系。
     *
     * @param screen 当前屏幕
     * @return true 如果是受支持的背包界面
     */
    private static boolean isSupportedInventoryScreen(net.minecraft.client.gui.screens.Screen screen) {
        return screen instanceof InventoryScreen || screen instanceof CreativeModeInventoryScreen;
    }

    /**
     * 获取背包界面的 GUI 宽度（imageWidth）
     * <p>
     * 生存模式 176px，创造模式 195px。
     */
    private static int getGuiWidth(net.minecraft.client.gui.screens.Screen screen) {
        if (screen instanceof CreativeModeInventoryScreen) return 195;
        return INV_WIDTH;
    }

    /**
     * 获取背包界面的 GUI 高度（imageHeight）
     * <p>
     * 生存模式 166px，创造模式 136px。
     */
    private static int getGuiHeight(net.minecraft.client.gui.screens.Screen screen) {
        if (screen instanceof CreativeModeInventoryScreen) return 136;
        return INV_HEIGHT;
    }
    ...
    private static int getLeftPos(net.minecraft.client.gui.screens.Screen screen) {
        return (screen.width - getGuiWidth(screen)) / 2;
    }
    ...
    private static int getTopPos(net.minecraft.client.gui.screens.Screen screen) {
        return (screen.height - getGuiHeight(screen)) / 2;
    }
    ...
    static void onScreenInit(ScreenEvent.Init.Post event) {
        if (!isSupportedInventoryScreen(event.getScreen())) return;
        ...
        int guiWidth = getGuiWidth(screen);
        int tabX = leftPos + guiWidth + 2;
    ...
    static void onScreenRender(ScreenEvent.Render.Post event) {
        ...
        // 计算面板位置
        int panelX = getLeftPos(screen) + getGuiWidth(screen) + 2;
    ...
    private static void renderPanel(GuiGraphicsExtractor g, int panelX, int panelY,
        ...
        // 3. 等级信息（通过 RPGSystems 获取等级附件，兼容客户端）
        SB.setLength(0);
        PlayerLevelData levelData = player.getData(com.rpgcraft.core.registry.RPGSystems.<PlayerLevelData>getPlayerLevelAttachment().get());
        ...
        // 4. 职业信息（通过 RPGSystems 获取职业附件和查询接口）
        ProfessionData profData = player.getData(RPGSystems.<ProfessionData>getPlayerProfessionAttachment().get());
        IProfession prof = RPGSystems.getProfessionSystem().getProfessionById(profData.getProfessionId());
    ...
    private static void renderAttributes(GuiGraphicsExtractor g, int x, int y,
        ...
        for (int i = 0; i < attrsPerPage; i++) {
            ...
            // 暴击率颜色：≤100 白色，101-200 橙色，201-300 红色
            int textColor = COLOR_TEXT;
            if (entry.getId().equals(AttributeManager.CRITICAL_RATE_ID)) {
                int critVal = attr.getValue();
            ...
            g.text(font, SB.toString(), attrX, attrY, textColor, false);
        }
    }
