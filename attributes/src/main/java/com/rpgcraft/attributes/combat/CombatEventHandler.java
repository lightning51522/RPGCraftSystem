...
public class CombatEventHandler {
    ...
    public static void onEntityJoinLevel(EntityJoinLevelEvent event) {
        ...
        Optional<MobAttributeConfig.MobAttributes> config = RPGSystems.getMobDataProvider().getConfig(typeId);
        if (config.isEmpty()) return;

        MobLevelData levelData = RPGSystems.getMobDataProvider().getMobLevelData(entity);
        ...
        } else if (CombatCommands.isRandomSpawnEnabled()) {
            // 随机刷新开启：从权重表中随机选择等级和评级
            MobAttributeConfig.SpawnDistribution dist = RPGSystems.getMobDataProvider().getSpawnDistribution(typeId);
            if (dist != null) {
    ...
    public static void initializeMobAttributesCustom(LivingEntity entity, int targetLevel,
        ...
        // 获取缩放器
        IMobAttributeScaler scaler = RPGSystems.getMobDataProvider().getScaler();
        ...
        // 失效快照缓存：新初始化的怪物属性已变更，确保后续快照从附件重新计算
        AttributeSnapshotManager.invalidate(entity);
    }
     ...
     * 将缩放后的属性值写入实体属性附件
     * <p>
     * 将固有基础值写入 {@link EntityAttributeAttachment}，
    ...
    public static void onLivingDamagePost(LivingDamageEvent.Post event) {
        ...
        if (target instanceof ServerPlayer serverPlayer) {
            RPGCraftCore.checkAndSnapshotIfDying(serverPlayer);
