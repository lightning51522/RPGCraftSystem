...
public class ProfessionManager {
     ...
     * 最后将系统接口注册到 {@link RPGSystems} 供其他模块访问。
     */
    public static void init() {
        ...
        // 注册附件 Supplier 到 RPGSystems（供客户端渲染访问）
        RPGSystems.registerPlayerProfessionAttachment(PLAYER_PROFESSION);

        // 注册到 RPGSystems 统一门面
        RPGSystems.registerProfessionSystem(new IProfessionSystem() {
            @Override
            ...
            @Override
            public IProfession getProfession(ServerPlayer player) {
                return ProfessionManager.getProfession(player);
            }

            @Override
            public IProfession getProfessionById(Identifier id) {
                return registry.getProfession(id);
            }

            @Override
            public Collection<IProfession> getAllProfessions() {
                return registry.getAllProfessions();
            }

            @Override
            public void setProfession(ServerPlayer player, Identifier professionId) {
     ...
     * 职业修饰符来源前缀
     */
    private static final String MODIFIER_PREFIX = "rpgcraftprofession";
    ...
    /**
     * 内部方法：应用或移除一组属性加成（使用修饰符模式）
     *
     * @param player   服务端玩家
     * @param bonusMap 属性加成映射
     * @param add      {@code true} 为添加修饰符，{@code false} 为移除修饰符
     */
    private static void applyBonus(ServerPlayer player, Map<Identifier, Integer> bonusMap, boolean add) {
        for (Map.Entry<Identifier, Integer> entry : bonusMap.entrySet()) {
            ...
            Identifier sourceId = modifierSourceId(entry.getKey());

            if (add) {
                attr.addModifier(AttributeModifier.of(sourceId, Operation.ADDITION, entry.getValue()));
            } else {
