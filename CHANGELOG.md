...
#### givegem 命令支持自定义词条数值（core / gemstone）

- 新命令语法：`/rpg gemstone givegem <宝石ID> <稀有度> <词条1> [词条1数值] [词条2] [词条2数值] [词条3] [词条3数值]`
- 每个词条后可选紧跟一个整数（任意整数含负数），覆盖默认查表值；省略则按宝石稀有度查默认数值表
- `GemInstance` 新增 `customValues` 字段（affixId → 数值），持久化到 Codec（`optionalFieldOf` 向后兼容旧存档）与 StreamCodec；属性词条与特效词条均可带数值
- 消费端（加成贡献者 / tooltip）查询时自定义数值优先，缺失才查默认表

### 修复

- **player 参数解析歧义**：`<词条> 10` 中的 `10` 被 `EntityArgument.player()` 贪婪消费误解析为玩家名（报"未找到玩家"）。根因是 `EntityArgument` parse 阶段消费任意 token 且不验证存在性，与 `IntegerArgumentType` 在同位置竞争。修复：从命令树彻底移除 player 参数，命令始终作用于执行者自己
- **负数数值显示**：tooltip 中词条数值为负数时显示「属性 +-N」，改为正数「+N」、负数「-N」
