...
> 另有 3 个**综合属性**（攻击力/魔法攻击/防御）不注册，由公式动态计算。
...
| `critical_rate` | 暴击率 | 5 | 300 | 暴击概率（百分比）；另受敏捷派生（每 5 敏捷 +1） |
| `critical_ratio` | 暴击伤害 | 50 | 无上限 | 每层暴击伤害加成（百分比）；另受精准派生（每 5 精准 +2） |
...
- **能力型**（`resetOnRespawn = false`，可加点）：力量、智力、敏捷、精准、法抗、暴击率、暴击伤害、固定伤害、物理穿透、法术穿透
...
有效暴击率 = 暴击率 + 敏捷/5（每 5 敏捷 +1）
有效暴击伤害 = 暴击伤害 + (精准/5)×2（每 5 精准 +2）
```
