...
**递增约定**：
- **哪些模块要升级**：仅升级**本次有改动的模块** + README 徽章；未涉及模块保持原版本号不动。
- **幅度选择**：
  - 跨模块大重构 / 新子系统 → 副版本 +1（如 `0.5.4` → `0.6.0`）
  - 单模块功能新增 / bug 修复 → 次版本 +1（如 `0.5.4` → `0.5.5`）
- **统一性**：被升级的模块使用**同一个**版本号，不允许某模块独立超前（如某子模块独自 `0.6.0` 而其它还在 `0.5.x`）。
